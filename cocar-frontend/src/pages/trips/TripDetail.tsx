...
export default function TripDetail() {
  ...
  useEffect(() => {
    // synchroniser la photo sélectionnée au chargement trip
    if (trip?.vehicle?.photos?.length) {
      const p = trip.vehicle.photos.find((x) => x.is_primary) || trip.vehicle.photos[0]
      setSelectedPhoto(p?.url || null)
    } else {
  ...
  const canBook = !isOwner && trip.available_seats > 0 && ['pending', 'confirmed'].includes(trip.status)
