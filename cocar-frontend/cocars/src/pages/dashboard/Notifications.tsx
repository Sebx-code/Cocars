...
export default function Notifications() {
  ...
  const getIcon = (type: string) => {
    switch (type) {
      ...
      case "new_rating": return <Star className="w-5 h-5 text-emerald-500" />;
      ...
      default: return <Bell className="w-5 h-5 text-theme-tertiary" />;
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-theme-tertiary">{unreadCount > 0 ? `${unreadCount} non lue(s)` : "Toutes lues"}</p>
        </div>
        {unreadCount > 0 && (
          <button
            onClick={markAllAsRead}
            className="flex items-center gap-2 px-5 py-2.5 text-emerald-700 hover:bg-emerald-50 rounded-full font-semibold transition-colors"
          >
      ...
      {!isLoading && notifications.length === 0 && (
        <div className="card-theme rounded-3xl border-2 p-16 text-center">
          ...
          <h3 className="text-2xl font-bold text-theme-primary mb-3">Aucune notification</h3>
          <p className="text-theme-tertiary">Vous n'avez pas de notification pour le moment.</p>
        </div>
      )}
        ...
        <div className="card-theme rounded-3xl border-2 overflow-hidden">
          {notifications.map((notification, index) => (
            <div
              ...
              className={`flex items-start gap-4 p-5 hover:bg-theme-secondary cursor-pointer transition-colors ${
                index !== notifications.length - 1 ? "border-b-2 border-theme" : ""
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2">
                  <p className={`font-semibold ${!notification.read ? "text-theme-primary" : "text-theme-secondary"}`}>
                    {notification.title}
                  </p>
                  <span className="text-xs text-theme-tertiary whitespace-nowrap">{formatTime(notification.created_at)}</span>
                </div>
